...
""")
    ...
    def signal_glue(self, portion_x1, portion_y1, portion_x2, portion_y2, order):
        ...
        if portion_x1[0] < portion_x2[0]:
            ...
            if gap:
                ...
                glued_x = np.concatenate([portion_x1, x_interp, portion_x2])
                glued_y = np.concatenate([portion_y1, y_interp, portion_y2])
            ...
            if gap:
                ...
                glued_x = np.concatenate([portion_x2, x_interp, portion_x1])
                glued_y = np.concatenate([portion_y2, y_interp, portion_y1])
                return glued_x, glued_y
            else:
                intersection = list(set(portion_x1) & set(portion_x2))
    ...
    def showGluedSignal(self):
        self.submit()
        size = 0
        glued_x, glued_y = self.signal_glue(self.portion_x1, self.portion_y1, self.portion_x2, self.portion_y2,
                                            self.interpolation_order)
        if len(glued_x) < len(glued_y):
            size = len(glued_x)
        else:
            size = len(glued_y)
        glued_x = glued_x[0:size]
        glued_y = glued_y[0: size]
        self.plot_graph_glued_signal.plot(glued_x, glued_y, pen=pg.mkPen(color="green", width=2.5))
